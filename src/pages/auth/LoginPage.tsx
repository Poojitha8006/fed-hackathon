...
const schema = z.object({ email: z.string().email(), password: z.string().min(6), role: z.enum(['admin','professional','user','support']) })
...
export default function LoginPage() {
  ...
  const onSubmit = (v: FormValues) => { login(v.email, v.role); navigate('/dashboard') }
  return (
    <div className="container-default py-10 max-w-md">
      ...
      <form className="mt-6 space-y-4" onSubmit={handleSubmit(onSubmit)}>
        ...
        <button className="rounded-md bg-primary-600 text-white px-4 py-2">Login</button>
      </form>
    </div>
  )
}
